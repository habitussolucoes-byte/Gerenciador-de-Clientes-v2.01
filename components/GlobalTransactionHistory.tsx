...
// Importing problematic functions and locales from subpaths to avoid index export issues
...
const GlobalTransactionHistory: React.FC<Props> = ({ clients }) => {
  ...
  // Use subpath import if named root export is missing
  const currentLocale = ptBR || ptBRLocale;
  ...
  const groupedTransactions = useMemo(() => {
    ...
    allTransactions.forEach(tx => {
      ...
      if (activeFilter === 'day') {
        ...
        label = format(date, 'MMMM yyyy', { locale: currentLocale });
  ...
  }, [allTransactions, activeFilter, currentLocale]);
  ...
  if (allTransactions.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-3xl border border-dashed border-gray-300 px-6">
        <p className="text-gray-400 font-black uppercase tracking-widest text-[10px]">Sem movimentação registrada</p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-br from-green-600 to-green-800 p-6 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden">
        <div className="flex justify-between items-center mb-4 relative z-10">
          <div>
            ...
            <h3 className="text-sm font-black uppercase">{format(now, 'MMMM yyyy', { locale: currentLocale })}</h3>
          </div>
          <button 
        ...
        {(['day', 'week', 'month'] as FilterType[]).map((filter) => (
          <button
            ...
            className={`flex-1 py-3.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${
              activeFilter === filter ? 'bg-white text-blue-600 shadow-md' : 'text-gray-400'
            }`}
          >
